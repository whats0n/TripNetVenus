...
;(() => {
  ...
  const items = $('.js-article');

  items.each((i, container) => {
    container = $(container);
    const control = container.find('.js-article-control');
    const content = container.find('.js-article-text');
  ...
  WIN.on('resize load', () => watch.forEach(fn => fn()));
